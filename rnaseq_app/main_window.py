...
class EnvSetupPage(QWidget):
        ...
        if ok:
            ...
            if state == "ok":
                ...
                self.conda_status_label.setText(
                    f"⚠ {info}  |  环境 '{env.env_name}' 残缺（创建中断的半成品），"
                    "点击下方按钮自动清理并重建"
                )
    ...
    def _create_env(self):
        ...
        if ok:
            self.conda_status_label.setText(f"✓ {msg}")
            self.conda_status_label.setStyleSheet(f"color: {COLORS['success']}; font-weight: bold;")
            self._env_ready = True
            self._refresh_env_buttons()
            self.create_env_btn.setText("重建环境")
    ...
    def _install_packages(self):
        ...
        def task_fn():
            ...
            # 磁盘空间预检：空间不足时安装中途失败会留下损坏的环境
            free_gb = env.free_disk_gb()
            if 0 <= free_gb < 2:
                self._install_results = [
                    (p.name, False, f"磁盘空间不足（仅 {free_gb:.1f} GB）") for p in PACKAGES
                ]
                return False, (f"磁盘剩余空间仅 {free_gb:.1f} GB，"
                               "完整软件包约需 5 GB。请清理磁盘（可用 conda clean -a 清缓存）后重试")
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        for i, step in enumerate(PIPELINE_STEPS):
            ...
            if required:
                ...
                marker_lbl.setStyleSheet("color: #b0b0b0; font-size: 14px;")
            step_grid.addWidget(marker_lbl, i, 0)
            ...
            step_grid.addWidget(status_lbl, i, 2)

            # 描述文字（浅色，填充剩余空间）
            desc_lbl = QLabel(step['description'])
            desc_lbl.setStyleSheet("color: #b0b0b0; font-size: 11px;")
            step_grid.addWidget(desc_lbl, i, 3)

        step_grid.setColumnStretch(3, 1)
        sg_layout.addLayout(step_grid)
        ...
        self.resume_btn.setToolTip("续跑模式：跳过已完成的步骤，自动重跑可能未完成的步骤")
    ...
    def _on_run_all(self, resume: bool = False):
        ...
        if resume:
                    ...
                    if last_dir:
                        ...
                        for s in affected:
                            ...
                            if s not in todo_steps:
                                todo_steps.append(s)
                            rerun_steps.append(s)

            if not todo_steps:
                QMessageBox.information(
                    self, "续跑",
                    "无可运行的步骤。如需重跑，请点击「从头运行」。"
                )
                return
            ...
            self._log(f"  将跳过 {len(done_steps)} 步，运行 {len(todo_steps)} 步")
            if rerun_steps:
                rerun_log = ", ".join(
