...
- **续跑预扫描与确认**：
  ...
  - 提示用户：如需重跑某步骤，手动删除对应文件夹后再次续跑即可
  - 例如删除 04_trinity_out 可基于步骤3的结果重跑步骤4
- **从头运行清空确认**：
...
- **样本配置表格自适应大小**：
  ...
  - 移除底部空白拉伸，配置区填满整个页面
- **执行步骤选择区域重做**：
  - 必需步骤前加 ★ 标记（Trinity/最长Isoform/CD-HIT），可选步骤无标记
  - 说明文字更清晰：★ 必需步骤不可取消，其余可选
  - 步骤选择区限制最大高度，不再挤压运行日志区
- **任务运行状态显示优化**：
  - 状态从难以理解的符号（○◉✓✗）改为文字标签：等待中 / 运行中… / 已完成 / 失败 / 已跳过
  - 运行中的步骤用蓝色背景高亮，已完成的用绿色背景，失败的用红色背景
  - 状态标签带圆角背景色，一眼可辨当前运行到哪一步
- **全局界面布局优化**：
  - 窗口默认尺寸调整为 1280×850，最小高度增至 750
  - Tab 标签加大内边距，选中项用主题色高亮
  - 面包屑导航栏加大内边距
  - 进度条加高至 28px，字体加粗
